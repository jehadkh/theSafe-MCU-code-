...
/*----------------------- Includes -----------------------*/
#include <avr/io.h>
#include "../../Header Files/mcal_layer/I2C.h"
...
/* ------------- Software Interfaces Defination ----------*/
	...
	void I2C_slave_write(unsigned char data)
	{
		...
		while(READ_BIT(TWCR,TWINT)==0);                     // Wait for TWINT flag
		while((TWSR & 0xF8) != 0xC0);   // Data byte in TWDR has been transmitted
	}
	...
	unsigned char I2C_Master_read(void)
	{
		TWCR=(1<<TWINT)|(1<<TWEN)|(1<<TWEA);
		while(READ_BIT(TWCR,TWINT)==0);
		while ((TWSR & 0XF8) != 0x40); // own SLA+R has been received and ACK has been returned
		TWCR=(1<<TWINT)|(1<<TWEN)|(1<<TWEA);
		while(READ_BIT(TWCR,TWINT)==0);
		while ((TWSR & 0XF8) != 0x50); // previously addressed with own SLA+R : data has been received : ACK has been returned
		return TWDR ;
	}
